...
img_path = 'C:/Users/Administrator/Desktop/2020sjkks/server/imgs/'
...
def goodinfo():
    ...
    sql2 = "SELECT user.uname,date_format(`order`.otime,'%Y-%m-%d'), order_detail.`comment`,uphoto FROM `user`,order_detail,`order` where order_detail.oid=order.oid and order.uid=user.uid and order_detail.`comment`!='' and order_detail.gid=" + str(
        good_id) + ";"
...
def order():
    ...
    sql = "INSERT INTO `meituan`.`order`( `uid`, `accepted`, `totalprice`,otime,address,phone) VALUES ( +" + str(
        uid) + ", 0, " + str(totalprice) + ",'" + dt + "','" + address + "'," + phone + ");"
    cursor.execute(sql)
    ...
    for key in selected.keys():  # 遍历点菜信息
        if (selected[str(key)] != 0):
            sql = "insert into order_detail values(" + str(oid) + "," + str(key) + ",null,null," + str(
                selected[str(key)]) + ")"
            cursor.execute(sql)
    try:
        db.commit()
...
def orderinfo():
    ...
    # print(data)
    oid = data['oid']
    sql = 'select gname,gphoto,gprice,quantity,otime,totalprice,accepted,good.gid,`order`.phone,`order`.address,courier,courier_phone from `order`,order_detail,good where good.gid=order_detail.gid and `order`.oid=order_detail.oid and `order`.oid=' + str(
        oid)
    cursor.execute(sql)
    db.commit()
    res = cursor.fetchall()
...
def set_somment():
    ...
    if score is not None and input is not None:
        print('3!')
        sql = "update order_detail set comment='" + input + "', score=" + str(score) + " where oid=" + str(
            oid) + " and gid=" + str(gid) + ";"
    try:
        cursor.execute(sql)
        db.commit()
...
def add_good():
    ...
    db = pymysql.connect(host='localhost', user=USER, password=PASSWORD, database=DATABASE)
    cursor = db.cursor()
    sql = "insert into good(gname,gdetail,gphoto,gprice,gonsale) values('" + gname + "','" + gdetail + "','" + gphoto + "'," + str(
        gprice) + ",1);"
    try:
        cursor.execute(sql)
        sql = "SELECT LAST_INSERT_ID();"  # 获取该订单的订单号
...
def edit_good():
    ...
    db = pymysql.connect(host='localhost', user=USER, password=PASSWORD, database=DATABASE)
    cursor = db.cursor()
    sql = "update good set gname='" + gname + "',gdetail='" + gdetail + "',gprice=" + str(
        gprice) + ",gphoto='" + gphoto + "' where gid=" + str(gid)
    try:
        cursor.execute(sql)
        db.commit()
        return 'succeed'
...
def be_a_courier():
    ...
    if (res[0][0] != 1):
        ...
        sql = "update `order` set accepted=2,courier=" + str(uid) + ",courier_phone='" + str(
            phone) + "' where oid=" + str(oid) + " ;"
        print(sql)
        try:
            cursor.execute(sql)
            db.commit()
